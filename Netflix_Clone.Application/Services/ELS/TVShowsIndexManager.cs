...
{
    public class TVShowsIndexManager(IElasticClient elasticClient,
        IOptions<ELSIndicesOptions> options) : ITVShowsIndexManager
    {
        ...
        {
            var createIndexResponse = await elasticClient.Indices.CreateAsync(IndexName ?? options.Value.TVShowsIndexName, c => c
                .Map<TVShowDocument>(m => m.AutoMap()));
        ...
        {
            var deleteIndexResponse = await elasticClient.Indices.DeleteAsync(IndexName ?? options.Value.TVShowsIndexName);
